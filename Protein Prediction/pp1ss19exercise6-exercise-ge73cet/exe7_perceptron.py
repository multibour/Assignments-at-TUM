...
class Perceptron:
    ...
    def _add_bias(self, arr):
        ...
        return np.append(arr, np.ones((arr.shape[0], 1)), axis=1)
    ...
    def _hinge_loss(self, y, y_pred):
        ...
        '''
        indices = (y != 0)
        hinge_loss = 1 - y_pred[indices] * y[indices]
    ...
    def _l2_loss(self, y, y_pred):
        ...
        '''
        l2_loss = 0.5 * np.average(np.square(y - y_pred))
    ...
    def multi_perceptron_with_bias(self):
        ...
        d_in = 3  # number of input nodes (again: 2 inputs + 1 bias)
        ...
        for epoch in range(self.nEpochs):  # for every epoch
            for index, x in enumerate(X):  # for every sample in the dataset
                ...
                h = x.reshape(1, -1) @ w1  # forward pass; inputs to hidden
                ...
                dw2 = grad_y_pred # grad_y_pred * w2
                dw1 = dw2 * w2 # dw2 * w1

                w1 -= self.learning_rate * x.reshape(-1, 1) @ dw1[:-1].reshape(1, -1)
                w2 -= self.learning_rate * dw2 * h_bias.reshape(-1)
    ...
    def multi_perceptron_with_bias_and_nonlinearity(self):
        ...
        for epoch in range(self.nEpochs):  # for every epoch
            for index, x in enumerate(X):  # for every sample in the data set
                ...
                d_sigm = h_non_lin * (1 - h_non_lin)

                w1 -= self.learning_rate * x.reshape(-1, 1) @ (dw1[:-1].reshape(1, -1) * d_sigm.reshape(1, -1))
                w2 -= self.learning_rate * dw2 * h_bias.reshape(-1)
