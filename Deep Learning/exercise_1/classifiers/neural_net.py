...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        y_pred = exponents / np.sum(exponents, axis=1, keepdims=True)

        reg_to_loss = 0.5 * reg * np.sum(np.square(W1)) + 0.5 * reg * np.sum(np.square(W2))
        loss = -np.sum(np.log(y_pred[np.arange(N), y])) / N + reg_to_loss
        ...
        dz = dscores @ W2.T  # (N, C) x (C, H) = (N, H)
        dz[z <= 0] = 0

        grads['W2'] = z.T @ dscores + reg * W2  # (H, N) x (N, C) = (H, C)
        grads['b2'] = np.sum(dscores, axis=0)  # (C,)

        grads['W1'] = X.T @ dz + reg * W1  # (D, N) x (N, H) = (D, H)
    ...
    def predict(self, X):
        ...
        z = np.maximum(X @ self.params['W1'] + self.params['b1'], 0)
        y_pred = np.argmax(z @ self.params['W2'] + self.params['b2'], axis=1)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    learning_rates = [1e-2, 1e-3, 4.3e-3, 4.5e-3, 4e-3, 3.7e-3, 3e-3, 2e-3, 1e-3]
    regularization_strengths = [0.4, 0.5, 0.6]

    for lr in learning_rates:
        for reg in regularization_strengths:
            classifier = TwoLayerNet(D, hidden_layer_size, C)
