...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        # max pool layer
        self.maxpool1 = nn.MaxPool2d(pool, stride=stride_pool)

        # linear layers
        padding = (kernel_size-1)/2
        height = int((height + 2 * padding - kernel_size) / stride_conv + 1)
        width = int((width + 2 * padding - kernel_size) / stride_conv + 1)
        self.fc1 = nn.Linear((height//pool) * (width//pool) * num_filters, hidden_dim)
